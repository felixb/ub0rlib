...
public abstract class TelephonyWrapper {
     ...
     *                    alphabet encoding are converted to as a single space characters. If false,
     *                    a messageBody containing non-GSM or non-CDMA alphabet characters are
     *                    encoded using 16-bit encoding.
     * @return an int[4] with int[0] being the number of SMS's required, int[1] the number of code
     * units used, and int[2] is the number of code units remaining until the next message. int[3]
     * is the encoding type that should be used for the message.
     */
     ...
     * @param scAddress          is the service center address or null to use the current default
     *                           SMSC
     * @param parts              an {@link ArrayList} of strings that, in order, comprise the
     *                           original message
     * @param sentIntents        if not null, an {@link ArrayList} of {@link PendingIntent}s (one
     *                           for each message part) that is broadcast when the corresponding
     *                           message part has been sent. The result code will be
     ...
     *                           sentIntent may include the extra "errorCode" containing a radio
     ...
     *                           against all unknown applicaitons, which cause smaller number of SMS
     *                           to be sent in checking period.
     * @param deliveryIntents    if not null, an ArrayList of PendingIntents (one for each message
     *                           part) that is broadcast when the corresponding message part has
     *                           been delivered to the recipient. The raw pdu of the status report
